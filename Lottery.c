  #include<stdio.h>
    ...
    //The file takes time until it runs but at the program shows a message at the end that indicates the running is done
         FILE * fp;
         fp = fopen ("/home/rana/Desktop/TheLotteryResults","w");
    ...
    if(t!=0)
    {
         tickets[t]=tickets[t-1]+inputs[b];
    }
    else
    {
        tickets[t]=inputs[b];
    ...
    int n = (rand() + lower) % (upper + 1);


    for(int kk=0;kk<finalsize;kk++)
    {
        if(n<tickets[kk])
           {
             i=kk;
    ...
    printf("\nAverage Turnaround time= %f\n",sum2*1.0/finalsize);
    printf("\n");
     printf("Done! You can now check the output on the desktop in the file TheLotteryResults :) !");
    fclose (fp);
